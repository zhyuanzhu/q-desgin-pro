<template>
    <div :class="[`${prefixCls}`]">
        <svg width="64" height="41" viewBox="0 0 64 41" xmlns="http://www.w3.org/2000/svg" v-if="empty && !error">
        ...
        <svg width="184" height="152" viewBox="0 0 184 152" xmlns="http://www.w3.org/2000/svg" v-if="error">
        ...
        <p :class="`${prefixCls}-text`" v-if="error">
            <slot name="error">
                <p>发生了未知异常~<a href="javscript" @click="handleClick">点击重试</a> </p>
            </slot>
        </p>
    </div>
</template>
